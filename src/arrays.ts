...
export function tripleNumbers(numbers: number[]): number[] {
    const tripled = numbers.map((word: number): number => word * 3);
    return tripled;
}
...
export function stringsToIntegers(numbers: string[]): number[] {
    const words = numbers.map((element: string): number => parseInt(element));
    return words;
    /**const iAmAnInt = numbers.map((words: string): number => parseInt(words));
    iAmAnInt.map((word: number): number =>
        !parseInt(word) ? (word = 0) : word
    );
    return iAmAnInt;
    */
}
...
export const removeDollars = (amounts: string[]): number[] => {
    const words = amounts.map((element: string): number => parseInt(element));
    return words;
...
export function countShortWords(words: string[]): number {
    const lessThanFour = words.filter(
        (lesser: string): boolean => lesser.length < 4
    );
